...
export default function FoodPage() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setResult(data)
      setLogs(l => [{ ...data, time: new Date().toLocaleTimeString('ja-JP') }, ...l])
    } catch (e) {
      console.error(e)
    } finally {
      setLoading(false)
        ...
        <div style={{padding:'16px'}}>
          ...
          <div style={{background:'#1e1e26',borderRadius:16,padding:'20px',border:'1px solid #2a2a36',marginBottom:16,textAlign:'center'}}>
            ...
            <label style={{display:'block',cursor:'pointer'}}>
              <div style={{
                background:'#25252f',border:'2px dashed #2a2a36',borderRadius:12,
                padding:'30px',marginBottom:12,
                color:image?'#39ff14':'#444',fontSize:13
              }}>
            ...
            <div style={{background:'#1e1e26',borderRadius:16,padding:'20px',border:'1px solid #2a2a36',marginBottom:16}}>
              ...
              {result.items && (
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:10,color:'#666',fontWeight:700,marginBottom:6}}>含まれる食材</div>
                  <div style={{display:'flex',flexWrap:'wrap',gap:4}}>
                    {result.items.map((item: string, i: number)=>(
                      <span key={i} style={{fontSize:11,background:'#25252f',border:'1px solid #2a2a36',borderRadius:6,padding:'3px 8px'}}>{item}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div style={{background:'#1e1e26',borderRadius:16,padding:'16px',border:'1px solid #2a2a36'}}>
              ...
              {logs.map((l,i)=>(
                <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #2a2a36'}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:700}}>{l.foodName}</div>
                    <div style={{fontSize:10,color:'#666'}}>{l.time}</div>
                  </div>
                  <div style={{textAlign:'right'}}>
              ...
              <div style={{marginTop:10,paddingTop:10,borderTop:'1px solid #2a2a36',display:'flex',justifyContent:'space-between'}}>
                <div style={{fontSize:12,color:'#666'}}>合計カロリー</div>
                <div style={{fontSize:16,fontWeight:800,color:'#ff8c00'}}>
