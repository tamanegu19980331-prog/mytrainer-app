...
export default function AdminPage() {
  ...
  const saveMenu = async () => {
    try {
      const exercises = menuForm.exercises ? JSON.parse(menuForm.exercises) : []
      const data = {
        name: menuForm.name, description: menuForm.description,
        category: menuForm.category, target_gender: menuForm.target_gender,
        target_age: menuForm.target_age, target_bmi: menuForm.target_bmi,
        target_goal: menuForm.target_goal, difficulty: menuForm.difficulty,
  ...
  const startEdit = (menu: any) => {
    ...
    setMenuForm({
      ...
      category: menu.category || '', target_gender: menu.target_gender || '',
      target_age: menu.target_age || '', target_bmi: menu.target_bmi || '',
      target_goal: menu.target_goal || '', difficulty: menu.difficulty || '',
      tags: menu.tags?.join(', ') || '',
  ...
  return (
    <div style={{ background: '#16161a', minHeight: '100vh', color: '#e8e8e8' }}>
      ...
      {/* ヘッダー */}
      <div style={{ padding: '14px 20px', borderBottom: '1px solid #2a2a36',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        background: '#1e1e26', position: 'sticky', top: 0, zIndex: 10 }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 800,
            color: '#ff4455', letterSpacing: 3 }}>⚙️ ADMIN PANEL</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)',
        ...
        {[
          ...
          <div key={k.label} style={{ background: '#1e1e26', borderRadius: 12,
            padding: '14px', border: '1px solid #2a2a36', textAlign: 'center' }}>
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        {tab === 'users' && (
          <div>
            <div style={{ fontSize: 11, color: '#666', marginBottom: 12 }}>
              登録ユーザー一覧（{users.length}名）· 名前をタップで詳細
            </div>
            {users.map(u => (
              <div key={u.id} style={{ background: '#1e1e26', borderRadius: 12,
                padding: '14px', border: '1px solid #2a2a36', marginBottom: 10 }}>
                ...
                <div onClick={() => goToUser(u.id)}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between',
                    ...
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 800,
                        color: '#00c8ff', textDecoration: 'underline' }}>
                        {u.name || '未設定'} →
                        {u.is_admin && (
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
                    {[
                      ...
                      <span key={item.l} style={{ fontSize: 10, background: '#25252f',
        ...
        {tab === 'menus' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between',
              ...
              <button onClick={() => {
                ...
                setMenuForm({ name:'',description:'',category:'',target_gender:'',
                  target_age:'',target_bmi:'',target_goal:'',difficulty:'',tags:'',exercises:'' })
                setShowMenuForm(true)
              }} style={{ padding: '8px 14px', background: '#39ff14', color: '#000',
            ...
            {showMenuForm && (
              <div style={{ background: '#1e1e26', borderRadius: 12, padding: '16px',
                border: '1px solid #39ff14', marginBottom: 16 }}>
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input style={inp} value={(menuForm as any)[key]}
                      onChange={e => setMenuForm(f => ({ ...f, [key]: e.target.value }))} />
                  </div>
                ))}
                <div>
                  <div style={{ fontSize: 10, color: '#666', marginBottom: 3 }}>
                  ...
                  <textarea style={{ ...inp, height: 120, resize: 'vertical',
                    ...
                    onChange={e => setMenuForm(f => ({ ...f, exercises: e.target.value }))} />
                </div>
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  <button onClick={saveMenu}
                    style={{ flex: 1, padding: '10px', background: '#39ff14',
                  ...
                  <button onClick={() => setShowMenuForm(false)}
                    style={{ flex: 1, padding: '10px', background: 'transparent',
                      color: '#666', border: '1px solid #2a2a36',
            ...
            {menus.map(m => (
              <div key={m.id} style={{ background: '#1e1e26', borderRadius: 12,
                padding: '14px',
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => startEdit(m)}
                      style={{ padding: '5px 10px', background: 'transparent',
                        ...
                        color: '#00c8ff', fontSize: 10, cursor: 'pointer' }}>
                      編集
                    </button>
                    <button onClick={() => deleteMenu(m.id)}
                      style={{ padding: '5px 10px', background: 'transparent',
                        border: '1px solid #ff4455', borderRadius: 6,
                        color: '#ff4455', fontSize: 10, cursor: 'pointer' }}>
                      削除
                    </button>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {[
                    ...
                    <span key={i} style={{ fontSize: 10, background: x.c + '20',
                      border: `1px solid ${x.c}44`, color: x.c,
                ...
                {m.exercises && (
                  <div style={{ fontSize: 10, color: '#444', marginTop: 6 }}>
                    種目数: {Array.isArray(m.exercises) ? m.exercises.length : '?'}件
                  </div>
                )}
              </div>
            ))}
        ...
        {tab === 'logs' && (
          <div>
            ...
            {logs.map(l => (
              <div key={l.id} style={{ background: '#1e1e26', borderRadius: 12,
                padding: '12px 14px', border: '1px solid #2a2a36', marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between',
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 10, color: l.completed ? '#39ff14' : '#666' }}>
                    {l.completed ? '✅ 完了' : '未完了'}
                  </span>
                </div>
              </div>
