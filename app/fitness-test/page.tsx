...
export default function FitnessTestPage() {
  ...
  const startCountdown = () => {
    ...
    intervalRef.current = setInterval(() => {
      c--
      setCountdown(c)
      if (c <= 3 && c > 0) playCountdownLast()
      else if (c > 3) playCountdown()
      if (c <= 0) {
    ...
    intervalRef.current = setInterval(() => {
      setTimer(t => {
        if (currentTest.type === 'count') {
          if (t <= 3 && t > 1) playWarning()
          if (t <= 1) { clearInterval(intervalRef.current); playFinish(); setPhase('input'); return 0 }
          return t - 1
        } else { return t + 1 }
      })
    }, 1000)
    ...
    <div style={{background:'#16161a',minHeight:'100vh',color:'#e8e8e8'}}>
      <div style={{maxWidth:480,margin:'0 auto',padding:'0 0 60px'}}>
        ...
        <div style={{padding:'32px 24px 0'}}>
          ...
          <button onClick={()=>{initAudio();setStep('test')}}
            style={{width:'100%',padding:'18px',background:'linear-gradient(135deg,#39ff14,#00c8ff)',color:'#000',border:'none',borderRadius:16,fontSize:16,fontWeight:800,cursor:'pointer',boxShadow:'0 4px 20px rgba(57,255,20,0.2)',marginBottom:12}}>
            テスト開始 →
          </button>
          <button onClick={()=>router.push('/menu')}
            style={{width:'100%',padding:'16px',background:'transparent',color:'#555',border:'1px solid #2a2a36',borderRadius:14,fontSize:13,cursor:'pointer'}}>
            スキップしてメニュー生成へ
          </button>
        </div>
      </div>
    ...
    <div style={{background:'#16161a',minHeight:'100vh',color:'#e8e8e8'}}>
      <div style={{maxWidth:480,margin:'0 auto',padding:'0 0 60px'}}>
        ...
        <div style={{padding:'0 24px'}}>
          <div style={{background:'#1e1e26',borderRadius:24,padding:'32px 24px',border:`1px solid ${currentTest.color}33`,marginBottom:16}}>
            ...
            {phase==='ready'&&(
              <div style={{textAlign:'center'}}>
                ...
                <button onClick={startCountdown}
                  style={{width:'100%',padding:'18px',background:currentTest.color,color:'#000',border:'none',borderRadius:14,fontSize:16,fontWeight:800,cursor:'pointer'}}>
                  スタート ▶
                </button>
              </div>
            )}
            ...
            {phase==='countdown'&&(
              <div style={{textAlign:'center'}}>
                ...
                <div style={{fontSize:112,fontWeight:800,lineHeight:1,color:countdown<=3?'#ff4455':currentTest.color,marginBottom:8,transition:'color 0.2s'}}>
                  {countdown}
                </div>
            ...
            {phase==='running'&&(
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:112,fontWeight:800,lineHeight:1,color:currentTest.type==='count'&&timer<=3?'#ff4455':currentTest.color,marginBottom:8,fontVariantNumeric:'tabular-nums',transition:'color 0.3s'}}>
                  {timer}
                </div>
                ...
                {currentTest.type==='time'&&(
                  <button onClick={stopTimer}
                    style={{width:'100%',padding:'18px',background:'#ff4455',color:'#fff',border:'none',borderRadius:14,fontSize:16,fontWeight:800,cursor:'pointer'}}>
                    ストップ ■
                  </button>
                )}
                {currentTest.type==='count'&&(
                  <div style={{background:'rgba(57,255,20,0.08)',borderRadius:12,padding:'14px',border:'1px solid rgba(57,255,20,0.2)',fontSize:14,color:'#39ff14',fontWeight:600}}>
                    全力で頑張ってください！
                  </div>
                )}
              </div>
            ...
            {phase==='input'&&(
              <div style={{textAlign:'center'}}>
                ...
                {currentTest.type==='count'?(
                  <div style={{marginBottom:16}}>
                    <input type="number" value={countInput}
                      ...
                      placeholder="回数を入力"
                      style={{width:'100%',background:'#25252f',border:`2px solid ${currentTest.color}`,borderRadius:14,padding:'20px',color:'#e8e8e8',fontSize:32,textAlign:'center',outline:'none',fontWeight:800}}
                    />
                  </div>
                ):(
                  <div style={{marginBottom:16}}>
                ...
                {(countInput||currentTest.type==='time')&&(
                  <div style={{background:'#25252f',borderRadius:14,padding:'16px',marginBottom:16}}>
                    {(()=>{
                ...
                <button
                  ...
                  style={{width:'100%',padding:'18px',background:currentTest.color,color:'#000',border:'none',borderRadius:14,fontSize:16,fontWeight:800,cursor:'pointer',opacity:currentTest.type==='count'&&!countInput?0.3:1}}>
                  次へ →
                </button>
              </div>
            )}
    ...
    <div style={{background:'#16161a',minHeight:'100vh',color:'#e8e8e8'}}>
      <div style={{maxWidth:480,margin:'0 auto',padding:'0 0 60px'}}>
        <div style={{padding:'20px 24px 0'}}>
          ...
          <div style={{background:'rgba(255,215,10,0.06)',borderRadius:20,padding:'24px',border:'1px solid rgba(255,215,10,0.15)',marginBottom:24,textAlign:'center'}}>
          ...
          <button onClick={saveResults} disabled={saving}
            style={{width:'100%',padding:'18px',background:'linear-gradient(135deg,#39ff14,#00c8ff)',color:'#000',border:'none',borderRadius:16,fontSize:16,fontWeight:800,cursor:'pointer',opacity:saving?0.5:1,marginBottom:12,boxShadow:'0 4px 20px rgba(57,255,20,0.2)'}}>
            {saving?'保存中...':'メニュー生成へ →'}
          </button>
